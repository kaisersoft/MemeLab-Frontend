...
(() => {
  ...
  function render() {
    ...
    const watchSub=document.querySelector("#watchlist-panel .panel-head > div > span");
    if(watchSub) watchSub.textContent="System-managed · ACTIVE & MATURE · 60s refresh";
    const maturePool=tokens.filter(t=>lifecycle(t)==="MATURE"||lifecycle(t)==="DECLINING");
    const matureCount=Math.min(maturePool.length,1000);
    const matureSub=document.querySelector(".mature-position-watchlist .panel-head > div > span");
    if(matureSub) matureSub.textContent=matureCount.toLocaleString("en-US")+" / 1,000 Mints · 60s refresh";
