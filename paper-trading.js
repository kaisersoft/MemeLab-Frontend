(() => {
  ...
  function renderSignals(){
    ...
    body.innerHTML=lastSignals.length ? lastSignals.map(x=>{
      ...
      return '<tr class="'+(eligible?"paper-eligible":"")+'"><td>'+(x.index+1)+'</td><td><strong>'+esc(t.symbol||"—")+'</strong><small>'+esc(t.name||"")+'</small></td><td class="paper-signal '+String(e.signal||"").toLowerCase()+'">'+esc(e.signal||"—")+'</td><td>'+ (e.strength??"—") +'</td><td>'+price(e.entry)+'</td><td>'+price(e.stop)+'</td><td>'+price(e.take)+'</td><td>'+((Number(t.core_score)||0)||"—")+'</td><td>'+(x.evaluatedAt?new Date(x.evaluatedAt).toLocaleTimeString():"—")+'</td></tr>';
  ...
  function renderJournal(){
    ...
    if(pnlBody)pnlBody.innerHTML=journal.length?journal.slice().reverse().map(p=>'<tr><td>'+p.id+'</td><td><strong>'+esc(p.symbol)+'</strong></td><td>BUY</td><td>'+esc(p.entryPriceText)+'</td><td>'+esc(p.exitPriceText)+'</td><td>'+esc(p.reason)+'</td><td class="'+((p.grossPnl??p.pnl)>=0?"paper-positive":"paper-negative")+'">'+usd(p.grossPnl??p.pnl)+'</td><td class="paper-negative">-'+usd(p.costTotal||0)+'</td><td class="'+((p.netPnl??p.pnl)>=0?"paper-positive":"paper-negative")+'">'+usd(p.netPnl??p.pnl)+'</td><td>'+pct(p.netPnlPct??p.pnlPct)+'</td></tr>').join(""):'<tr><td colspan="10" class="paper-empty">No closed trades recorded.</td></tr>';
  }
